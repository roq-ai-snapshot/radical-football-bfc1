...
import { getParentPlayers, deleteParentPlayerById } from 'apiSdk/parent-players';
...
function ParentPlayerListPage() {
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteParentPlayerById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (id: string) => {
    if (hasAccess('parent_player', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/parent-players/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('parent_player', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Link href={`/parent-players/create`}>
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('parent_player', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <Td>
                          <NextLink href={`/parent-players/edit/${record.id}`} passHref legacyBehavior>
                            <Button
                          ...
                          {hasAccess('parent_player', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                            <IconButton
                              onClick={(e) => {
                                e.stopPropagation();
